# -*- ruby -*-

require 'rubygems'
require 'hoe'
require './lib/idn_actionmailer.rb'

Hoe.new('IdnActionmailer', IdnActionmailer::VERSION) do |p|
  p.author = 'Mick Staugaard'
  p.email = 'mick@staugaard.com'
  p.summary = 'monkey path for ActionMailer to support international domain names'
  p.description = 'monkey path for ActionMailer to support international domain names'
  p.url = 'http://github.com/staugaard/idn_actionmailer'
  p.changes = p.paragraphs_of('History.txt', 0..1).join("\n\n")
  #p.extra_deps << ['mime-types']
end

# vim: syntax=Ruby
